...
export class FixMissingColumns1757197200000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        // Agregar columnas faltantes a la tabla sensores (solo si existe)
        const sensoresExists = await queryRunner.hasTable("sensores");
        if (sensoresExists) {
            await queryRunner.query(`ALTER TABLE "sensores" ADD COLUMN IF NOT EXISTS "latitud" decimal(8,4) DEFAULT NULL`);
            await queryRunner.query(`ALTER TABLE "sensores" ADD COLUMN IF NOT EXISTS "longitud" decimal(8,4) DEFAULT NULL`);
        }
        
        // Actualizar columnas NOT NULL en lotes (si existe)
        ...
        // Actualizar columnas NOT NULL en categorias (si existe)
        ...
        // Actualizar columnas NOT NULL en almacenes (si existe)
        const almacenesExists = await queryRunner.hasTable("almacenes");
        if (almacenesExists) {
            await queryRunner.query(`UPDATE "almacenes" SET "nombre_almacen" = 'sin_nombre' WHERE "nombre_almacen" IS NULL`);
            await queryRunner.query(`ALTER TABLE "almacenes" ALTER COLUMN "nombre_almacen" SET NOT NULL`);
            await queryRunner.query(`UPDATE "almacenes" SET "descripcion" = 'sin_descripcion' WHERE "descripcion" IS NULL`);
            await queryRunner.query(`ALTER TABLE "almacenes" ALTER COLUMN "descripcion" SET NOT NULL`);
        }

        // Actualizar columnas en usuarios (solo numero_documento que es nullable) si existe
        const usuariosExists = await queryRunner.hasTable("usuarios");
        if (usuariosExists) {
            await queryRunner.query(`UPDATE "usuarios" SET "numero_documento" = 'sin_numero' WHERE "numero_documento" IS NULL`);
            await queryRunner.query(`ALTER TABLE "usuarios" ALTER COLUMN "numero_documento" SET NOT NULL`);
        }
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        // Revertir cambios
        await queryRunner.query(`ALTER TABLE "sensores" DROP COLUMN IF EXISTS "longitud"`);
        await queryRunner.query(`ALTER TABLE "sensores" DROP COLUMN IF EXISTS "latitud"`);
        ...
        await queryRunner.query(`ALTER TABLE "almacenes" ALTER COLUMN "nombre_almacen" DROP NOT NULL`);
        await queryRunner.query(`ALTER TABLE "almacenes" ALTER COLUMN "descripcion" DROP NOT NULL`);
        await queryRunner.query(`ALTER TABLE "usuarios" ALTER COLUMN "numero_documento" DROP NOT NULL`);
    }
}
