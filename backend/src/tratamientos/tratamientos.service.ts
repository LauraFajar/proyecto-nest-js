...
export class TratamientosService {
  ...
  async create(
    ...
    try {
      const epa = await queryRunner.manager.findOne(Epa, {
        where: { id_epa: tratamientoData.id_epa },
      });
      if (!epa) {
        throw new NotFoundException(
          `Epa con ID ${tratamientoData.id_epa} no encontrado`,
        );
      }

      const nuevoTratamiento = queryRunner.manager.create(Tratamiento, {
        ...tratamientoData,
        id_epa: epa,
      });
      const tratamientoGuardado =
        await queryRunner.manager.save(nuevoTratamiento);

      if (insumos && insumos.length > 0) {
        for (const insumoDto of insumos) {
          await this.ajustarInventarioYRelacion(
            queryRunner,
            tratamientoGuardado.id_tratamiento,
            insumoDto,
            true,
          );
        }
  ...
  private async ajustarInventarioYRelacion(
    queryRunner: any,
    id_tratamiento: number,
    insumoDto: TratamientoInsumoDto,
    esNuevo: boolean,
  ) {
    const insumo = await queryRunner.manager.findOne(Insumo, {
      where: { id_insumo: insumoDto.id_insumo },
    });
    if (!insumo)
      throw new NotFoundException(
        `Insumo con ID ${insumoDto.id_insumo} no encontrado`,
      );
    ...
    if (!inventario)
      throw new NotFoundException(
        `No existe inventario para el insumo ${insumo.nombre_insumo}`,
      );
    ...
    if (stockActual < cantidadRequerida) {
      throw new BadRequestException(
        `Stock insuficiente para ${insumo.nombre_insumo}. Disponible: ${stockActual}, Requerido: ${cantidadRequerida}`,
      );
    }
  ...
  async findOne(id_tratamiento: number) {
    // Esta función no necesita cambios y se mantiene como estaba
    try {
      const tratamiento = await this.tratamientosRepository
        .createQueryBuilder('tratamiento')
        .leftJoinAndSelect('tratamiento.id_epa', 'epa')
        .leftJoinAndSelect(
          'tratamiento.tratamientoInsumos',
      ...
      console.error(
        `Error al obtener tratamiento con ID ${id_tratamiento}:`,
        error,
      );
  ...
  async update(
    ...
    try {
      ...
      if (insumos) {
        const insumosActuales = await queryRunner.manager.find(
          TratamientoInsumo,
          {
            where: { id_tratamiento },
        ...
        const insumosActualesMap = new Map(
          insumosActuales.map((ti) => [ti.id_insumos.id_insumo, ti]),
        );
        ...
        for (const actual of insumosActuales) {
          if (!insumosNuevosMap.has(actual.id_insumos.id_insumo)) {
            ...
            if (
              !insumoEntity.es_herramienta &&
              insumoEntity.tipo_insumo !== 'herramienta'
            ) {
              const inventario = await queryRunner.manager.findOne(Inventario, {
                where: { id_insumo: insumoEntity.id_insumo },
              });
              if (inventario) {
                inventario.cantidad_stock =
                  Number(inventario.cantidad_stock) +
                  Number(actual.cantidad_usada);
                await queryRunner.manager.save(inventario);
              }
            }
        ...
        for (const insumoDto of insumos) {
          const insumoEntity = await queryRunner.manager.findOne(Insumo, {
            where: { id_insumo: insumoDto.id_insumo },
          });
          if (!insumoEntity)
            throw new NotFoundException(
          ...
          if (diferencia !== 0) {
            ...
            if (!inventario)
              throw new NotFoundException(
                `Inventario para el insumo ${insumoEntity.nombre_insumo} no encontrado.`,
              );

            const stockActual = Number(inventario.cantidad_stock);
            if (stockActual < diferencia) {
              throw new BadRequestException(
                `Stock insuficiente para ${insumoEntity.nombre_insumo}. Disponible: ${stockActual}, ajuste requerido: ${diferencia}`,
              );
            }
          ...
          if (actual) {
            ...
            actual.unidad_medida =
              insumoDto.unidad_medida || actual.unidad_medida;
      ...
      await queryRunner.commitTransaction();
      const updatedTratamiento = await this.tratamientosRepository.findOne({
        where: { id_tratamiento },
        relations: [
          'id_epa',
          'tratamientoInsumos',
          'tratamientoInsumos.id_insumos',
        ],
      });
      if (!updatedTratamiento) {
        throw new InternalServerErrorException(
          'No se pudo encontrar el tratamiento después de la actualización.',
        );
      }
  ...
  async remove(id_tratamiento: number): Promise<void> {
    ...
    try {
      ...
      for (const tratamientoInsumo of tratamiento.tratamientoInsumos) {
        ...
        if (!insumo.es_herramienta && insumo.tipo_insumo !== 'herramienta') {
          ...
          if (inventario) {
            inventario.cantidad_stock =
              Number(inventario.cantidad_stock) +
              Number(tratamientoInsumo.cantidad_usada);
            await queryRunner.manager.save(inventario);
          }
        }
      }
      ...
      throw new InternalServerErrorException(
        'Error al eliminar el tratamiento: ' + error.message,
      );
    } finally {
      await queryRunner.release();
    }
