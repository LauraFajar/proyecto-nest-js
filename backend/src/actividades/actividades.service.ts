...
export class ActividadesService {
  ...
  async create(createActividadDto: CreateActividadeDto) {
    ...
    if (!cultivoExists) {
      throw new NotFoundException(
        `Cultivo con ID ${createActividadDto.id_cultivo} no encontrado`,
      );
    }
    ...
    const actividadData: Partial<Actividad> = {
      ...
      costo_mano_obra:
        createActividadDto.costo_mano_obra != null
          ? String(createActividadDto.costo_mano_obra)
          : undefined,
      costo_maquinaria:
        createActividadDto.costo_maquinaria != null
          ? String(createActividadDto.costo_maquinaria)
          : undefined,
      horas_trabajadas:
        createActividadDto.horas_trabajadas != null
          ? String(createActividadDto.horas_trabajadas)
          : undefined,
      tarifa_hora:
        createActividadDto.tarifa_hora != null
          ? String(createActividadDto.tarifa_hora)
          : undefined,
    };

    if (
      actividadData.costo_mano_obra == null &&
      createActividadDto.horas_trabajadas != null &&
      createActividadDto.tarifa_hora != null
    ) {
    ...
    if (saved.responsable_id) {
      try {
        const fechaStr =
          saved.fecha instanceof Date
            ? saved.fecha.toISOString().slice(0, 10)
            : new Date(saved.fecha).toISOString().slice(0, 10);
        const detalle =
          saved.detalles && String(saved.detalles).trim().length
            ? saved.detalles
            : saved.tipo_actividad;
        await this.alertasService.crearNotificacion(
          Number(saved.responsable_id),
          'Nueva Actividad',
          `Se te asignó la actividad "${detalle}" para el ${fechaStr}`,
          'actividad_asignada',
        );
        await this.alertasService.notificarAdministradores(
          'Nueva Actividad',
          `Se asignó la actividad "${detalle}" al usuario ${String(saved.responsable || saved.responsable_id)} para el ${fechaStr}`,
          'actividad_asignada',
        );
      } catch {}
    }

    const recursos = (createActividadDto as any).recursos as
      | Array<{
          id_insumo: number;
          cantidad?: number;
          horas_uso?: number;
    ...
    if (Array.isArray(recursos) && recursos.length > 0) {
      ...
      for (const r of recursos) {
        ...
        const utiliza = this.utilizaRepository.create({
          ...
          costo_unitario:
            r.costo_unitario != null ? String(r.costo_unitario) : undefined,
        });
        ...
        if (insumo.es_herramienta || insumo.tipo_insumo === 'herramienta') {
          ...
          if (horas > 0) {
            ...
            const acumNext = Math.min(
              costoCompra || Infinity,
              acumPrev + depActividad,
            );
            ...
            const costoHora =
              r.costo_unitario != null
                ...
                : Number(insumo.depreciacion_por_hora || 0);
            if (costoHora > 0) {
          ...
          if (cant > 0) {
            ...
            if (!inventarioItem) {
              console.error(
                `Inventario para el insumo ${insumo.nombre_insumo} no encontrado al crear actividad.`,
              );
              continue;
            }
            try {
              ...
              await this.salidasService.create({
                ...
                cantidad: cant,
                observacion: `Salida autom. act. ID ${saved.id_actividad}`,
                fecha_salida: new Date(createActividadDto.fecha)
                  .toISOString()
                  .slice(0, 10),
              ...
              console.error(
                'Error al crear movimiento automático de salida en create (ActividadesService):',
                err,
              );
            }
          }
      ...
      const sumaMaquinaria =
        Number(saved.costo_maquinaria || 0) +
        Number(totalDepreciacion || 0) +
        Number(totalHerramientasCosto || 0);
      if (sumaMaquinaria > 0) {
  ...
  async handlePhotoUpload(actividadId: number, photo: Express.Multer.File) {
    ...
    const filePathFromMulter = photo?.path
      ? path.join(process.cwd(), photo.path)
      : null;
    if (
      filePathFromMulter &&
      !fs.existsSync(filePathFromMulter) &&
      photo?.buffer
    ) {
  ...
  async update(
    ...
    try {
      ...
      const prevEstado = actividad.estado;
      const prevResponsableId = actividad.responsable_id;
      ...
      if (Array.isArray(recursos)) {
        ...
        const actualesMap = new Map(
          recursosActuales.map((u) => [u.id_insumo.id_insumo, u]),
        );
        const nuevosMap = new Map(
          (recursos as RecursoDto[]).map((r) => [r.id_insumo, r]),
        );
        ...
        for (const actual of recursosActuales) {
          if (!nuevosMap.has(actual.id_insumo.id_insumo)) {
            ...
            if (
              !insumo.es_herramienta &&
              insumo.tipo_insumo !== 'herramienta'
            ) {
        ...
        for (const recursoDto of recursos as RecursoDto[]) {
          const insumo = await queryRunner.manager.findOne(Insumo, {
            where: { id_insumo: recursoDto.id_insumo },
          });
          if (!insumo) continue;
          ...
          if (insumo.es_herramienta || insumo.tipo_insumo === 'herramienta') {
            ...
            if (actual) {
              actual.cantidad =
                recursoDto.cantidad != null
                  ? String(recursoDto.cantidad)
                  : undefined;
              actual.horas_uso =
                recursoDto.horas_uso != null
                  ? String(recursoDto.horas_uso)
                  : undefined;
              actual.costo_unitario =
                recursoDto.costo_unitario != null
                  ? String(recursoDto.costo_unitario)
                  : undefined;
            ...
            if (inventario) {
              ...
              if (diferencia > 0 && stockActual < diferencia) {
                throw new BadRequestException(
                  `Stock insuficiente para ${insumo.nombre_insumo}.`,
                );
              }

              if (diferencia > 0) {
                const inventarioItem = await queryRunner.manager.findOne(
                  Inventario,
                  { where: { id_insumo: insumo.id_insumo } },
                );
                if (!inventarioItem) {
                  throw new InternalServerErrorException(
                    `Inventario para el insumo ${insumo.nombre_insumo} no encontrado al actualizar actividad.`,
                  );
                }
                try {
                  const valorUnidad =
                    recursoDto.costo_unitario != null
                      ? Number(recursoDto.costo_unitario)
                      : null;
                  console.log(
                    'DEBUG: Buscando salida existente para actualizar:',
                    {
                      id_insumo: insumo.id_insumo,
                  ...
                  const salidaExistente = await queryRunner.manager.findOne(
                    Salida,
                    {
                      where: {
                  ...
                  if (salidaExistente) {
                    console.log(
                      'DEBUG: Actualizando salida existente ID:',
                  ...
                  console.error(
                    'Error al actualizar/crear salida automática en update (ActividadesService):',
                    err,
                  );
                  throw new InternalServerErrorException(
                    'Error al registrar la salida automática: ' + err.message,
                  );
                }
              }
              ...
              throw new NotFoundException(
                `Inventario para el insumo ${insumo.nombre_insumo} no encontrado.`,
              );
            }
          }
          ...
          if (actual) {
            ...
            if (cantidadNueva > 0) {
              ...
              if (!inventarioItem) {
                throw new InternalServerErrorException(
                  `Inventario para el insumo ${insumo.nombre_insumo} no encontrado al asociar nuevo insumo.`,
                );
              }
              try {
                const valorUnidad =
                ...
                await this.salidasService.create(
                  {
                    ...
                    observacion: `Salida autom. nueva asoc. act. ${actividad.id_actividad}`,
                    fecha_salida: new Date().toISOString().slice(0, 10),
                    unidad_medida: inventarioItem.unidad_medida,
                    id_cultivo: actividad.id_cultivo,
                    valor_unidad: valorUnidad ?? undefined,
                ...
                console.error(
                  'Error al crear movimiento automático de salida para nuevo insumo en update (ActividadesService):',
                  err,
                );
                throw new InternalServerErrorException(
                  'Error al registrar la salida automática para nuevo insumo: ' +
                    err.message,
                );
              }
            }
      ...
      if (!updatedActividad) {
        throw new InternalServerErrorException(
          'No se pudo encontrar la actividad después de la actualización.',
        );
      }
      if (prevEstado !== updatedActividad.estado) {
        try {
          const fechaStr =
            updatedActividad.fecha instanceof Date
              ? updatedActividad.fecha.toISOString().slice(0, 10)
              : new Date(updatedActividad.fecha).toISOString().slice(0, 10);
          const detalle =
            updatedActividad.detalles &&
            String(updatedActividad.detalles).trim().length
              ? updatedActividad.detalles
              : updatedActividad.tipo_actividad;
          const destinatario =
            Number(updatedActividad.responsable_id || prevResponsableId);
          if (destinatario) {
            await this.alertasService.crearNotificacion(
              destinatario,
              'Estado de Actividad',
              `La actividad "${detalle}" cambió su estado a "${updatedActividad.estado}" (fecha: ${fechaStr})`,
              'actividad_estado',
            );
          }
          await this.alertasService.notificarAdministradores(
      ...
      if ((updateActividadDto as any)?.responsable_id) {
        try {
          const fechaStr =
            updatedActividad.fecha instanceof Date
              ? updatedActividad.fecha.toISOString().slice(0, 10)
          ...
          const detalle =
            ...
            String(updatedActividad.detalles).trim().length
              ...
              : updatedActividad.tipo_actividad;
          await this.alertasService.crearNotificacion(
            Number((updateActividadDto as any).responsable_id),
            'Actividad Actualizada',
            `Se te asignó/actualizó la actividad "${detalle}" para el ${fechaStr}`,
            'actividad_asignada',
          );
        } catch {}
      }
  ...
  async getRecursosByActividad(actividadId: number) {
    ...
    return recursos.map((u) => {
      ...
      let es_herramienta =
        (u.horas_uso != null && String(u.horas_uso) !== '') ||
        rawFlag === true ||
        rawFlag === 1 ||
        rawFlag === '1' ||
        (typeof rawFlag === 'string' &&
          (rawFlag.toLowerCase() === 'true' || rawFlag.toLowerCase() === 't'));
      if (!es_herramienta) {
        const catName = (ins?.id_categoria?.nombre || '')
          .toString()
          .toLowerCase();
        if (
      ...
      return {
        ...
        costo_unitario:
          u.costo_unitario != null ? Number(u.costo_unitario) : undefined,
      };
    });
  }

  async removeFoto(fotoId: number): Promise<void> {
    const foto = await this.fotoActividadRepository.findOne({
      where: { id: fotoId },
    });
    if (!foto) {
    ...
    try {
      ...
      console.error(
        `Error al borrar el archivo físico ${foto.url_imagen}:`,
        error,
      );
    }
  ...
  async remove(id_actividad: number): Promise<void> {
    ...
    try {
      ...
      const fotos = await queryRunner.manager.find(FotoActividad, {
        where: { actividad: { id_actividad } },
      });
      ...
      throw new InternalServerErrorException(
        'Error al eliminar la actividad: ' + error.message,
      );
    } finally {
      await queryRunner.release();
    }
  ...
  async obtenerReporteActividades(
    ...
    if (tipo_actividad) {
      query.andWhere('actividad.tipo_actividad = :tipo_actividad', {
        tipo_actividad,
      });
    }
  ...
  async obtenerEstadisticas() {
    ...
    const estadisticas = {
      ...
      por_estado: {
        completadas: actividades.filter((a) => a.estado === 'completada')
          .length,
        pendientes: actividades.filter((a) => a.estado === 'pendiente').length,
        en_progreso: actividades.filter((a) => a.estado === 'en_progreso')
          .length,
      },
      costo_promedio: 0,
  ...
  private agruparPorCultivo(actividades: Actividad[]) {
    return actividades.reduce(
      (acc, actividad) => {
        const cultivoId =
          actividad.cultivo?.id_cultivo?.toString() || 'Sin cultivo';
