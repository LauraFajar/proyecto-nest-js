...
export class MigracionMaestra1761200000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (sensoresExists) {
      const hasTipoSensor = await queryRunner.hasColumn(
        'sensores',
        'tipo_sensor',
      );
      if (!hasTipoSensor) {
        await queryRunner.query(
          `ALTER TABLE "sensores" ADD COLUMN "tipo_sensor" character varying`,
        );
      }
      ...
      if (!hasUbicacion) {
        await queryRunner.query(
          `ALTER TABLE "sensores" ADD COLUMN "ubicacion" character varying`,
        );
      }
      ...
      if (!hasEstado) {
        await queryRunner.query(
          `ALTER TABLE "sensores" ADD COLUMN "estado" character varying DEFAULT 'activo'`,
        );
      }
    }

    if (sensoresExists) {
      const hasIdSublote = await queryRunner.hasColumn(
        'sensores',
        'id_sublote',
      );
      if (!hasIdSublote) {
        await queryRunner.query(
          `ALTER TABLE "sensores" ADD COLUMN "id_sublote" integer`,
        );
        await queryRunner.query(
          `CREATE INDEX IF NOT EXISTS "IDX_sensores_id_sublote" ON "sensores" ("id_sublote")`,
        );
      }
    }
    ...
    if (usuariosExists) {
      const hasImagenUrl = await queryRunner.hasColumn(
        'usuarios',
        'imagen_url',
      );
      if (!hasImagenUrl) {
        await queryRunner.query(
          `ALTER TABLE "usuarios" ADD COLUMN "imagen_url" character varying`,
        );
      }
    }
  }
