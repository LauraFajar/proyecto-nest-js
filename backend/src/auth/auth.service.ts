...
export class AuthService {
  ...
  async register(createUserDto: CreateUsuarioDto) {
    const existingUserByDoc = await this.usersService.findOneByDocumento(
      createUserDto.numero_documento,
    );
    ...
    if (!defaultRole) {
      throw new BadRequestException(
        'El rol Invitado no está configurado en el sistema',
      );
    }
  ...
  async forgotPassword(
    ...
    if (!user) {
      return {
        message:
          'Si el correo electrónico existe, recibirás un enlace de recuperación.',
      };
    }
