...
import { View, Text, Modal, TextInput, Pressable, StyleSheet, FlatList, Alert, ScrollView } from 'react-native';
  ...
  const renderPerm = ({ item }) => {
    ...
    return (
      <View style={[styles.permRow, isHighlighted ? styles.highlightRow : null]}>
        <Text style={[styles.permCell, styles.wRes]}>{item.recurso}</Text>
        <Text style={[styles.permCell, styles.wAcc]}>{item.accion}</Text>
        <Text style={[styles.permCell, styles.wName]}>{displayName}</Text>
        <Text style={[styles.permCell, styles.wState]}>{item.activo ? 'Activo' : 'Inactivo'}</Text>
        <View style={[styles.permCell, styles.wAssigned]}>
          <Pressable
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 4 }}>
            <View style={{ minWidth: 860 }}>
              <View style={styles.tableHeader}>
                <Text style={[styles.headerCell, styles.wRes]}>Recurso</Text>
                <Text style={[styles.headerCell, styles.wAcc]}>Acción</Text>
                <Text style={[styles.headerCell, styles.wName]}>Nombre</Text>
                <Text style={[styles.headerCell, styles.wState]}>Estado</Text>
                <Text style={[styles.headerCell, styles.wAssigned]}>Asignado</Text>
                <Text style={[styles.headerCell, styles.wAction]}>Acción</Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  headerCell: { flex: 1, fontSize: 12, fontWeight: '700', color: '#16A34A', paddingHorizontal: 8 },
