        ...
        {mapData.map((lote) => {
          ...
          return (
            <React.Fragment key={`lote-${lote.id_lote || lote.id}`}>
              {showLotes && lotePositions.length >= 3 && lotPassesFilter(lote) && (enabledLots[lid] ?? true) && (
                <PolygonCmp
...
const styles = StyleSheet.create({
  ...
  listCard: { width: '92%', maxWidth: 360, maxHeight: '70%', backgroundColor: '#fff', borderRadius: 12, overflow: 'hidden', borderWidth: 2, borderColor: '#3B82F6' },
