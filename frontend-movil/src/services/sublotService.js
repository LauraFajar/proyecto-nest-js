...
const sublotService = {
  ...
  updateCoordinates: async (token, id, geometry) => {
    try {
      const coords =
        geometry && Array.isArray(geometry.coordinates)
          ? geometry.type === 'Polygon'
            ...
            : null
          : null;
      const body = coords ? { coordenadas: coords } : null;
      if (!body) {
      ...
      const data = await res.json();
      if (!res.ok) {
        throw new Error(data?.message || 'Error al actualizar coordenadas de sublote');
      }
      return data;
    } catch (error) {
  ...
  updateCoordinates: async (token, id, geometry) => {
    try {
      ...
      if (!r3.ok) {
        throw new Error('Error al actualizar coordenadas del sublote');
      }
