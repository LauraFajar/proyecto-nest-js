...
export async function createMovimiento(payload, token) {
  const normalizeTipo = (t) => {
    ...
    return x === 'salida' ? 'Salida' : 'Entrada';
  };
  ...
  const body = {
    ...
    tipo_movimiento: normalizeTipo(payload.tipo_movimiento),
    cantidad: Number(payload.cantidad || 0),
    unidad_medida: (() => {
      const raw = String(payload.unidad_medida || payload.unidad || '').trim();
      const val = raw.length ? raw : 'unidad';
      return val.slice(0, 20);
    })(),
    fecha_movimiento: toDateInput(payload.fecha_movimiento || payload.fecha),
  ...
  const res = await fetch(getMovimientosUrl(), { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify(body) });
  const ct = res.headers.get('content-type') || '';
  const data = ct.includes('application/json') ? await res.json() : await res.text();
...
export async function updateMovimiento(id, payload, token) {
  const normalizeTipo = (t) => {
    if (t === undefined || t === null || String(t).trim().length === 0) return undefined;
    const x = String(t || '').trim().toLowerCase();
    return x === 'salida' ? 'Salida' : 'Entrada';
  };
  const toDateInput = (value) => {
  ...
  const body = {
    ...(payload.id_insumo !== undefined ? { id_insumo: Number(payload.id_insumo) } : {}),
    ...(payload.tipo_movimiento !== undefined ? { tipo_movimiento: normalizeTipo(payload.tipo_movimiento) } : {}),
    ...(payload.cantidad !== undefined ? { cantidad: Number(payload.cantidad || 0) } : {}),
    ...(payload.unidad_medida !== undefined || payload.unidad !== undefined
      ? {
          unidad_medida: (() => {
            const raw = String(payload.unidad_medida || payload.unidad || '').trim();
            const val = raw.length ? raw : 'unidad';
      ...
      : {}),
    ...(payload.fecha_movimiento !== undefined || payload.fecha !== undefined ? { fecha_movimiento: toDateInput(payload.fecha_movimiento || payload.fecha) } : {}),
    responsable: payload.responsable,
    observacion: payload.observacion,
    ...(payload.id_cultivo !== undefined ? { id_cultivo: payload.id_cultivo != null ? Number(payload.id_cultivo) : undefined } : {}),
    ...(payload.valor_unidad !== undefined ? { valor_unidad: payload.valor_unidad != null ? Number(payload.valor_unidad) : undefined } : {}),
  };
  const res = await fetch(`${getMovimientosUrl()}/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify(body) });
  const ct = res.headers.get('content-type') || '';
  const data = ct.includes('application/json') ? await res.json() : await res.text();
