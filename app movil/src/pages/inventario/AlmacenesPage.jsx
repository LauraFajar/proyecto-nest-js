...
import { listAlmacenes, createAlmacen, updateAlmacen, deleteAlmacen } from '../../services/api';
import AlmacenFormModal from '../../components/molecules/AlmacenFormModal';
...
export default function AlmacenesPage() {
  ...
  const fetchData = async () => {
    setError('');
    try {
      const arr = await listAlmacenes(token);
      setItems(arr.map((a) => ({ id: a.id, nombre: a.nombre, descripcion: a.descripcion })));
    } catch (e) {
  ...
  return (
    <View style={styles.root}>
      <View style={styles.header}>
        ...
        <Pressable style={styles.newBtn} onPress={() => { setCurrent(null); setOpenForm(true); }}><Feather name="plus" size={16} color="#fff" /><Text style={styles.newBtnText}> Nuevo Almacén</Text></Pressable>
      </View>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <Row
                  ...
                  onDelete={() => setConfirm({ open: true, text: `¿Eliminar almacén ${item.nombre}?`, action: async () => { await deleteAlmacen(item.id, token); fetchData(); } })}
                />
              )}
      ...
      <AlmacenFormModal visible={openForm} almacen={current} onClose={() => setOpenForm(false)} onSubmit={async (payload) => { if (current) await updateAlmacen(current.id, payload, token); else await createAlmacen(payload, token); fetchData(); }} />
...
const styles = StyleSheet.create({
  root: { flex: 1, backgroundColor: '#fff', padding: 12, paddingBottom: 40 },
